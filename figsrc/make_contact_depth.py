...
fig.text(0.035, 0.950, 'The detected pixel agrees with the arm. The depth along that pixel’s ray does not.',
         fontsize=14, weight='bold')
fig.text(0.035, 0.892, 'At the contact frame, over 58 contacts in 8 venues. The ray is the tracker’s raw detection; the body never sees a shuttle.',
...
A.text(2.0, 3.30, 'line of sight through the raw detection', fontsize=8.6, style='italic',
...
A.text(5.00, 4.60, 'where the reconstruction puts the shuttle along the ray:\nIQR −1.11 to +1.65 m, median bias +0.21 m',
...
A.text(W[0] - 0.30, (W[1] + Q[1]) / 2, '0.52 m', fontsize=10.0, color=ACC2, weight='bold', ha='right')
...
A.text(9.15, 1.55, '1.54 m', fontsize=10.0, color=MUT, weight='bold')
...
dist = np.array([1.08, 0.74, 0.52, 0.66, 0.95])
B.axhspan(0.44, 0.68, color=WARM, zorder=0)
B.plot(off, dist, marker='o', ms=5.5, color=ACC, lw=1.6, zorder=3)
B.plot([0], [0.52], marker='o', ms=9, mfc=WARM, mec=ACC2, mew=2.0, zorder=4)
B.text(8.2, 0.465, 'one racket', fontsize=8.0, color=ACC2, ha='right', va='center')
B.set_xticks(off); B.set_xlabel('frames from the contact', fontsize=9, labelpad=4)
B.set_ylabel('nearest wrist to the ray (m)', fontsize=9)
B.set_ylim(0.30, 1.25); B.tick_params(labelsize=8.5, length=3)
for sp in ('top', 'right'):
